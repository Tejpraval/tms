...
export default function SystemAuditPage() {
    ...
    const fetchAuditLogs = async () => {
        ...
        try {
            ...
            // Handling both plain array return and object wrappers
            const logs = Array.isArray(data) ? data : data.data || [];
            setEvents(logs);
        } catch (err: any) {
            console.error("Failed to fetch audit stream", err);
            setError(err.message || "Failed to load system audit stream.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const tenants = useMemo(() => Array.from(new Set(events.map(e => e.tenantId).filter(Boolean))), [events]);
    ...
    }, [events, filterTenant, filterAction, filterOutcome]);
        ...
        <div className="p-8 text-white max-w-7xl mx-auto space-y-6">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center bg-zinc-900 p-6 rounded-lg border border-zinc-800 gap-4">
                <div>
                    <h1 className="text-2xl font-bold font-mono text-zinc-100 flex items-center gap-2">
                        <span className="text-emerald-500 animate-pulse">●</span> LIVE AUDIT STREAM
                    </h1>
                    <p className="text-zinc-400 text-sm mt-1">Real-time unredacted security ledger across all operational tenants.</p>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                    <select
                        ...
                        className="bg-black border border-zinc-800 rounded px-3 py-1.5 text-sm text-zinc-300 focus:outline-none focus:border-blue-500"
                    >
            ...
            {loading && events.length === 0 ? (
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-12 text-center">
                    ...
                    <p className="text-zinc-500 text-sm mt-2">No infrastructure events match the current filter criteria.</p>
                </div>
            ) : (
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                            {filteredEvents.map(event => (
                                <tr key={event._id} className="hover:bg-zinc-800/30 transition-colors font-mono text-xs">
                                    ...
                                    <td className="px-6 py-3 text-zinc-500">{event.tenantId || '--'}</td>
